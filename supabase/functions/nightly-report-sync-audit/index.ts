...
//
// (2026-09-17, 리포트 동기화 안정화 3단계 중 마지막 단계) 전체 등록을 매번 다시 스캔하는 온시마리 
// cron 대신, "전송로그(학원) DB"에서 오늘 성공적으로 발송된 보고서만 훑어서 그 등록들만 다시
// 재동기화한다. send-report가 발송 직전에 이미 한 번 강제 재동기화하지만(ensureFreshReportCache),
// 혹시 그 사이 편집이 있었거나 재동기화 자체가 조용히 실패했을 경우를 대비한 마지막 안전망이다.
// 스캔 범위가 "오늘 전송된 건수"로 고정되어 있어서, 등록 수가 계속 늘어나도 이 함수의 비용은
// 늘어나지 않는다.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ checkedLogs: logs.length, registrations: ids.length, synced: syncedCount, errors: errorCount }),
      { headers: { ...ADMIN_CORS, "Content-Type": "application/json" } },
    )
  } catch (err) {
