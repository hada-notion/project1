// _shared/generateTuitionTarget.ts
//
// generate-tuition이 처리하는 실제 수강료 생성 로직을 별도 파일로 분리했다 (2026-09-22, Phase 6:
// 동시성 제어 -- 큐 도입). 원래 supabase/functions/generate-tuition/index.ts 안에 있던 코드를
...
import { getPage, createPage, archivePage, mapWithConcurrency } from "./notionClient.ts"
...
export const CLASS_TUITION_STATUS_SPEC: StatusSpec = {
...
// process-sync-queue 워커가 target: "generate-tuition" 작업을 처리할 때 호출하는 진입점.
export const processGenerateTuitionQueueItem = makeQueuedBackgroundProcessor(
	"generate-tuition",
	CLASS_TUITION_STATUS_SPEC,
	"classId",
	processClass,
)
