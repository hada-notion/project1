// _shared/generateReportTarget.ts
//
// generate-report가 처리하는 실제 보고서 생성 로직을 별도 파일로 분리했다 (2026-09-22, Phase 6:
// 동시성 제어 -- 큐 도입). 원래 supabase/functions/generate-report/index.ts 안에 있던 코드를
// 그대로 옮긴 것이다. index.ts는 이제 웹훅 payload 파싱 + 큐 적재만 담당한다.
//
...
import { getPage, createPage, queryAllPages, archivePage, mapWithConcurrency } from "./notionClient.ts"
import { makeQueuedBackgroundProcessor } from "./webhookIngest.ts"
import { PROP_LAST_ERROR } from "./constants.ts"
...
export const CLASS_REPORT_STATUS_SPEC: StatusSpec = {
...
// process-sync-queue 워커가 target: "generate-report" 작업을 처리할 때 호출하는 진입점.
export const processGenerateReportQueueItem = makeQueuedBackgroundProcessor(
	"generate-report",
	CLASS_REPORT_STATUS_SPEC,
	"classId",
	processClass,
)
